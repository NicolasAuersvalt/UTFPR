...
// Declara o tipo de cada bola, que sera utilizado
// para os centros de cada uma
typedef struct {
  int x_inicial;
  int y_inicial;
  int x_final;
...
double detectaSensorBar(Imagem1C *img, Coordenada *l, Coordenada *r) {
  ...
  // Altera o L e o R passados por referencia
...
agrupamentos de brancos **/
void encontraCentros(Imagem1C *img, int *rotulo_maior, Coordenada_bola *bola) {

  /* Percorre toda a Matriz buscando pelos rotulos laterais
     Ao encontrar o rotulo, armazena a primeira ocorrencia dele,
     ou seja, a primeira aparicao pela esquerda, pela direita,
  ...
  */
  ...
  for (j = 0; j < img->largura-1; j++) {
    for (i = 0; i < img->altura-1; i++) {
      ...
      // Se encontrar o rotulo maior e a flag de encontro for valida
      if ((img->dados[i][j] == *rotulo_maior) && (achou)) {

        // Armazena a posicao da ocorrencia de branco
        bola->x_inicial = j;

        // Impossibilita encontrar outro rotulo pelo lado
        achou = 0;
      }
    }
  }
  ...
  // Inicia a procura ja pela direita, pois esta sendo
  // procurado rotulo a direita
...
double calculaAngulo(Coordenada_bola *bola1, Coordenada_bola *bola2) {
  ...
  // Calcula o modulo do vetor posicao

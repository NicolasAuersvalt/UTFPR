...
void destroiLabirinto (Labirinto* lab)
{
    // Precisa acessar a matriz antes de desalocar os campos

    // Desaloca cada linha da matriz
    for(int linha=0; linha < lab.h; linha++){
        free(lab.mat[linha]);    
}
    free(lab.h);
    free(lab.w);
/*
  //nao ficaria melhor? (verificar validade)
  int i;
  for(i = 0; i < lab -> h; i++)
    free(lab -> mat[i]);
  free (lab -> mat);
...
void preencheMatrizCusto (Labirinto* lab, Coordenada2D pos_queijo)
{
    int contador_casas = 0, mudou = 1;

    while(mudou!=0){

        mudou = 1;

        for(int linha = 1; linha < (lab->w - 1); linha++){

            for(int coluna= 1; coluna< (lab->h - 1); coluna++){

                if(lab->mat[linha, coluna] == contador_casas){

                    // Verifica se os laterais estão disponíveis

                    if(linha < lab->h && lab->mat[linha+1][coluna] == CAMINHO){
                        lab->mat[linha+1][coluna] = contador_casas+1;
                        mudou++;
                    }

                    if(linha > 0 && lab->mat[linha-1][coluna] == CAMINHO){
                        lab->mat[linha-1][coluna] = contador_casas+1;
                        mudou++;
                    }

                    if(coluna < lab->w && lab->mat[linha][coluna+1] == CAMINHO){
                        lab->mat[linha][coluna+1] = contador_casas+1;
                        mudou++;
                    }

                    if(coluna > 0 && lab->mat[linha][coluna-1] == CAMINHO){
                        lab->mat[linha][coluna-1] = contador_casas+1;
                        mudou++;
                    }
                    if(mudou)
